class Api::V1::DishController < ApplicationController
  ...
  def set_rating
    ...
    if top_five_id = params[:top_five_id] and !top_five_id.blank? and Dishcoin.where(top_five_id:top_five_id, user_id: user.id).count < 1 and top_five = TopFive.where(id:top_five_id).first
  ...
  def log_view
    ...
    if session and session[:session_id]
      dish_view.session_id = session[:session_id]
    end
