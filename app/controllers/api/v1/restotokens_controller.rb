class Api::V1::RestotokensController  < ApplicationController
  ...
  def create
    ...
    if @user.owns_restaurants.size == 0 
      logger.info("No restaurants assigned to user #{@user.to_json}. Please contact support.")
      render :status=>402, :json=>{:message=>"No restaurants assigned to user #{@user.to_json}. Please contact support."}
      return
    end  

    if @user.owns_restaurants.size > 1 
      logger.info("Too many restaurants assigned to user #{@user.to_json}")
      render :status=>403, :json=>{:message=>"Too many restaurants assigned to user. Please contact support."}
      return
    end  

    restaurant = @user.owns_restaurants.first

    reso = {:foodcloud_token=>@user.authentication_token, :restaurant => restaurant.as_json}
